...
/* ------------------------------------ */
if ( ! function_exists( 'clearer_hex2rgb' ) ) {

	function clearer_hex2rgb( $hex, $array=false ) {
		$hex = str_replace("#", "", $hex);
...
/*  Google fonts
/* ------------------------------------ */
if ( ! function_exists( 'clearer_enqueue_google_fonts' ) ) {

	function clearer_enqueue_google_fonts () {
...
add_action( 'wp_enqueue_scripts', 'clearer_enqueue_google_fonts' ); 	


/*  Dynamic css output
/* ------------------------------------ */
if ( ! function_exists( 'clearer_dynamic_css' ) ) {

	function clearer_dynamic_css() {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
		
			// rgb values
			$color_1 = get_theme_mod('color-1');
			$color_1_rgb = clearer_hex2rgb($color_1);
...
.wrapper-inner { max-width: '.esc_attr( get_theme_mod('container-width') ).'px; }					
			...
			// content max-width
			if ( get_theme_mod('content-width','774') != '774' ) {
				$styles .= '
.entry-header,
.entry-footer,
...
.entry-header .entry-featured-image img { border-radius: '.esc_attr( get_theme_mod('thumb-border-radius') ).'px; }
			...
			// color hover text
			if ( get_theme_mod('color-hover-text','#3399ff') != '#3399ff' ) {
				$styles .= '
...
.widget_nav_menu > div > ul li a:hover:before { background: '.esc_attr( get_theme_mod('color-hover-text') ).'; }
			...
			// color more link
			if ( get_theme_mod('color-more-link','#e2e7ef') != '#e2e7ef' ) {
				$styles .= '
.more-link-custom { background: '.esc_attr( get_theme_mod('color-more-link') ).'!important; color: #fff!important; }
				'."\n";
			}
			// color comment bubble
			if ( get_theme_mod('color-comment-bubble','#ffffff') != '#ffffff' ) {
				$styles .= '
.comments-bubble i { color: '.esc_attr( get_theme_mod('color-comment-bubble') ).'!important; }
.comments-bubble span { color: #fff!important; }
				'."\n";
			}
			// background color
			if ( get_theme_mod('color-background','#fbfbfc') != '#fbfbfc' ) {
				$styles .= '
body { background: '.esc_attr( get_theme_mod('color-background') ).'; }
			...
			if ( get_theme_mod('dark','off') == 'on' ) { 
				wp_add_inline_style( 'clearer-dark', $styles );
			} else {
				wp_add_inline_style( 'clearer-style', $styles );
			}
		}
	}
	
}
add_action( 'wp_enqueue_scripts', 'clearer_dynamic_css' );
