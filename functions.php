...
if ( ! function_exists( 'clearer_load' ) ) {
	
	function clearer_load() {
		// Load theme languages
		load_theme_textdomain( 'clearer', get_template_directory().'/languages' );
...
add_action( 'after_setup_theme', 'clearer_load' );	
...
/*  Theme setup
/* ------------------------------------ */
if ( ! function_exists( 'clearer_setup' ) ) {
	
	function clearer_setup() {
		// Enable title tag
		...
		// Thumbnail sizes
		add_image_size( 'clearer-small', 320, 180, true );
		...
		register_nav_menus( array(
			'mobile' 	=> esc_html__( 'Mobile', 'clearer' ),
			'header' 	=> esc_html__( 'Header', 'clearer' ),
		) );
	}
	
}
add_action( 'after_setup_theme', 'clearer_setup' );


/*  Custom navigation
/* ------------------------------------ */
if ( ! class_exists( '\Clearer\Nav' ) ) {
...
/*  Custom logo
/* ------------------------------------ */
if ( ! function_exists( 'clearer_custom_logo' ) ) {
	
	function clearer_custom_logo() {
		$defaults = array(
			'height'		=> 120,
			'width'			=> 400,
			'flex-height'	=> true,
...
add_action( 'after_setup_theme', 'clearer_custom_logo' );


/*  Custom header
/* ------------------------------------ */
if ( ! function_exists( 'clearer_custom_header' ) ) {
	
	function clearer_custom_header() {
		$args = array(
			'default-image'	=> false,
			'default-text'	=> false,
			'width'			=> 1120,
...
add_action( 'after_setup_theme', 'clearer_custom_header' );


/*  Custom background
/* ------------------------------------ */
if ( ! function_exists( 'clearer_custom_background' ) ) {
	
	function clearer_custom_background() {
		$args = array();
		add_theme_support( 'custom-background', $args );
	}
	
}
add_action( 'after_setup_theme', 'clearer_custom_background' );


/*  Deregister
/* ------------------------------------ */
if ( ! function_exists( 'clearer_deregister' ) ) {
	
	function clearer_deregister() {
		wp_deregister_style( 'wp-pagenavi' );
	}
	
}
add_action( 'wp_enqueue_scripts', 'clearer_deregister', 100 );


/*  Register sidebars
/* ------------------------------------ */	
if ( ! function_exists( 'clearer_sidebars' ) ) {

	function clearer_sidebars()	{
		register_sidebar(array( 'name' => esc_html__('Primary','clearer'),'id' => 'primary','description' => esc_html__("Normal full width sidebar","clearer"), 'before_widget' => '<div id="%1$s" class="widget %2$s">','after_widget' => '</div>','before_title' => '<h3>','after_title' => '</h3>'));
		
		if ( get_theme_mod('footer-widgets') >= '1' ) { register_sidebar(array( 'name' => esc_html__('Footer 1','clearer'),'id' => 'footer-1', 'description' => esc_html__("Widgetized footer","clearer"), 'before_widget' => '<div id="%1$s" class="widget %2$s">','after_widget' => '</div>','before_title' => '<h3>','after_title' => '</h3>')); }
		if ( get_theme_mod('footer-widgets') >= '2' ) { register_sidebar(array( 'name' => esc_html__('Footer 2','clearer'),'id' => 'footer-2', 'description' => esc_html__("Widgetized footer","clearer"), 'before_widget' => '<div id="%1$s" class="widget %2$s">','after_widget' => '</div>','before_title' => '<h3>','after_title' => '</h3>')); }
		if ( get_theme_mod('footer-widgets') >= '3' ) { register_sidebar(array( 'name' => esc_html__('Footer 3','clearer'),'id' => 'footer-3', 'description' => esc_html__("Widgetized footer","clearer"), 'before_widget' => '<div id="%1$s" class="widget %2$s">','after_widget' => '</div>','before_title' => '<h3>','after_title' => '</h3>')); }
		if ( get_theme_mod('footer-widgets') >= '4' ) { register_sidebar(array( 'name' => esc_html__('Footer 4','clearer'),'id' => 'footer-4', 'description' => esc_html__("Widgetized footer","clearer"), 'before_widget' => '<div id="%1$s" class="widget %2$s">','after_widget' => '</div>','before_title' => '<h3>','after_title' => '</h3>')); }
	}
	
}
add_action( 'widgets_init', 'clearer_sidebars' );


/*  Enqueue javascript
/* ------------------------------------ */	
if ( ! function_exists( 'clearer_scripts' ) ) {
	
	function clearer_scripts() {
		wp_enqueue_script( 'clearer-slick', get_template_directory_uri() . '/js/slick.min.js', array( 'jquery' ),'', false );
...
add_action( 'wp_enqueue_scripts', 'clearer_scripts' ); 


/*  Enqueue css
/* ------------------------------------ */	
if ( ! function_exists( 'clearer_styles' ) ) {
	
	function clearer_styles() {
		wp_enqueue_style( 'clearer-style', get_stylesheet_uri() );
		wp_enqueue_style( 'clearer-responsive', get_template_directory_uri().'/responsive.css' );
		if ( ( get_theme_mod( 'dark-theme','off' ) == 'on' ) || ( get_theme_mod( 'theme-toggle','on' ) == 'on' ) ) { wp_enqueue_style( 'clearer-dark', get_template_directory_uri().'/dark.css' ); }
		wp_enqueue_style( 'clearer-font-awesome', get_template_directory_uri().'/fonts/all.min.css' );
	}
	
}
add_action( 'wp_enqueue_scripts', 'clearer_styles' );
...
/*  Layout class
/* ------------------------------------ */
if ( ! function_exists( 'clearer_layout_class' ) ) {
	
	function clearer_layout_class() {
		// Default layout
		$layout = 'col-2cl';
		$default = 'col-2cl';
...
/* ------------------------------------ */
if ( ! function_exists( 'clearer_sidebar_primary' ) ) {
	
	function clearer_sidebar_primary() {
		// Default sidebar
		$sidebar = 'primary';
...
/*  Social links
/* ------------------------------------ */
if ( ! function_exists( 'clearer_social_links' ) ) {

	function clearer_social_links() {
...
/*  Site name/logo
/* ------------------------------------ */
if ( ! function_exists( 'clearer_site_title' ) ) {

	function clearer_site_title() {
...
/*  Blog title
/* ------------------------------------ */
if ( ! function_exists( 'clearer_blog_title' ) ) {

	function clearer_blog_title() {
		global $post;
...
/*  Related posts
/* ------------------------------------ */
if ( ! function_exists( 'clearer_related_posts' ) ) {

	function clearer_related_posts() {
		wp_reset_postdata();
		global $post;
...
/* ------------------------------------ */
if ( ! function_exists( 'clearer_get_featured_post_ids' ) ) {

	function clearer_get_featured_post_ids() {
		$args = array(
...
/*  Body class
/* ------------------------------------ */
if ( ! function_exists( 'clearer_body_class' ) ) {
	
	function clearer_body_class( $classes ) {
		$classes[] = clearer_layout_class();
...
if ( ! function_exists( 'clearer_excerpt_more' ) ) {
	...
	function clearer_excerpt_more( $more ) {
		if ( is_admin() ) {
			return $more;
		}
		return '&#46;&#46;&#46;';
...
add_filter( 'excerpt_more', 'clearer_excerpt_more' );


/*  Excerpt length
/* ------------------------------------ */
if ( ! function_exists( 'clearer_excerpt_length' ) ) {

	function clearer_excerpt_length( $length ) {
		if ( is_admin() ) {
			return $length;
		}
...
add_filter( 'excerpt_length', 'clearer_excerpt_length', 999 );
...
/* ------------------------------------ */
if ( ! function_exists( 'clearer_pre_get_posts' ) ) {

	function clearer_pre_get_posts( $query ) {
		...
		if ( $query->is_home() ) {
			...
			if ( get_theme_mod('featured-posts-count','4') != '0' ) {
				// Get featured post ids
				$featured_post_ids = clearer_get_featured_post_ids();
				// Exclude posts
...
add_action( 'pre_get_posts', 'clearer_pre_get_posts' );


/*  Script for no-js / js class
/* ------------------------------------ */
if ( ! function_exists( 'clearer_html_js_class' ) ) {

	function clearer_html_js_class () {
		echo '<script>document.documentElement.className = document.documentElement.className.replace("no-js","js");</script>'. "\n";
	}
	
}
add_action( 'wp_head', 'clearer_html_js_class', 1 );


/*  Admin panel css
/* ------------------------------------ */
if ( ! function_exists( 'clearer_admin_panel_css' ) ) {
	
	function clearer_admin_panel_css() {
		global $pagenow;
...
add_action( 'admin_head', 'clearer_admin_panel_css' );


/*  TGM plugin activation
/* ------------------------------------ */
if ( ! function_exists( 'clearer_plugins' ) ) {
	
	function clearer_plugins() {	
		if ( get_theme_mod('recommended-plugins','on') =='on' ) { 	
			// Add the following plugins
			$plugins = array(
				array(
					'name' => esc_html__( 'Alx Extensions', 'clearer' ),
					'slug' => 'alx-extensions',
				),
				array(
					'name' => esc_html__( 'Meta Box', 'clearer' ),
					'slug' => 'meta-box',
				),
				array(
					'name' => esc_html__( 'Regenerate Thumbnails', 'clearer' ),
					'slug' => 'regenerate-thumbnails',
				),
				array(
					'name' => esc_html__( 'WP-PageNavi', 'clearer' ),
					'slug' => 'wp-pagenavi',
				)
			);	
			tgmpa( $plugins );
...
add_action( 'tgmpa_register', 'clearer_plugins' );


/*  WooCommerce basic support
/* ------------------------------------ */
function clearer_wc_wrapper_start() {
	echo '<div class="content">';
}
function clearer_wc_wrapper_end() {
	echo '</div>';
}
remove_action( 'woocommerce_before_main_content', 'woocommerce_output_content_wrapper', 10);
remove_action( 'woocommerce_after_main_content', 'woocommerce_output_content_wrapper_end', 10);
add_action('woocommerce_before_main_content', 'clearer_wc_wrapper_start', 10);
add_action('woocommerce_after_main_content', 'clearer_wc_wrapper_end', 10);


/*  Accessibility IE11 fix - https://git.io/vWdr2
/* ------------------------------------ */
function clearer_skip_link_focus_fix() {
	?>
	<script>
